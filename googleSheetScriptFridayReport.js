 ...
 */
 const copyTableFromTemplatesToOfficial = (sheetName, templateRowRange) => {
    ...
    const rowCountToInsert = templateRowRangeArray[1] - templateRowRangeArray[0] + 1  // 1 is row gap between tables
    ...
    let topLeftCornerRange = spreadsheet.getActiveSheet().getRange(`${sheetName}!1:1`);
    spreadsheet.getActiveSheet().setActiveRange(topLeftCornerRange);
    spreadsheet.getRange(`Table templates!${templateRowRange}`).copyTo(spreadsheet.getActiveRange(), SpreadsheetApp.CopyPasteType.PASTE_NORMAL, false);
    spreadsheet.getRange(`Table templates!${templateRowRange}`).copyTo(spreadsheet.getActiveRange(), SpreadsheetApp.CopyPasteType.PASTE_VALUES, false);
